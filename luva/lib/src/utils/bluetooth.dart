...
class BluetoothConnectionHandler extends ChangeNotifier
{
  ...
  Future<bool> _scanDevices() async 
  {
    ...
    {
      flutter_console.log("Bluetooth Scan Timeout");
      event.close();
    })
    .take(10)
    .firstWhere((element)
    {
      flutter_console.log("Find Device => ${element.name}");
    ...
    onError: (error)
    {
      flutter_console.logError("Error on discovering device => $error");
      return;
    });
  ...
  Future<void> _connectToDevice() async
  {
    ...
    .listen((event) async
    {
      ...
      {
        flutter_console.logError("Error on receiving Bluetooth Connection Update => $err");
      }
    },
    onError: (error)
    {
      flutter_console.logError("Error on listening for Bluetooth Connection Update => $error");
      _currentBluetoothConnectionState = BluetoothConnectionState.deviceDisconnected;
      notifyListeners();
    });
